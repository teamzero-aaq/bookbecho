	...
	class WooCommerceCheckOutForm extends FormHandler implements IFormHandler
	{
		...
		function handle_woocommere_checkout_form($getdata)
		{
			...
			if(strcasecmp($this->_otpType,$this->_typePhoneTag)==0)
				$this->sendChallenge('test',$getdata['user_email'],null, trim($getdata['user_phone']),"phone");
			else
				$this->sendChallenge('test',$getdata['user_email'],null,null,"email");
		}
                    ...
                    $mo(".validate-required").each(function(){
                        ...
                        if(n!="") {
                            ...
                            if((val=="" || val=="select") && checkOptionalMandatoryFields(n) ) {
                                ...
                                errors  += "<li><strong>"+$mo("#"+n+"_field").children("label").text().replace("*","")+"</strong> is a required field.</li>";
                            }
                        }
						}
                    });
		...
		{
			if(strcasecmp($this->_otpType,$this->_typePhoneTag)==0) {
                echo '<div title="' . mo_("Please Enter a Phone Number to enable this link") . '">
                ...
                echo '<div title="' . mo_("Please Enter an Email Address to enable this link") . '">
                            <a  href="#" 
		...
		function handle_otp_token_submitted($error)
		{
			...
			if(!$error) $this->validateChallenge('order_verify');
		}
		...
		function handleFormOptions()
		{
			...
			update_mo_option('wc_checkout_payment_type',$this->_paymentMethods);
		}
